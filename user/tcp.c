...
static void conn_packet(struct connection *c, const struct pkt_tcp *p,
    const struct tcp_opts *opts, uint16_t flow_group);
    ...
    uint16_t remote_port, uint32_t db_id, struct connection **pconn)
{
  ...
  if (ret < 0) {
    fprintf(stderr, "%s: nicif_arp failed\n", __func__);
    conn_free(conn);
    return -1;
  } else if (ret == 0) {
...
int tcp_packet(const void *pkt, uint16_t len, uint16_t flow_group)
{
  struct connection *c;
  struct listener *l;
  ...
  if ((c = conn_lookup(p)) != NULL) {
    conn_packet(c, p, &opts, flow_group);
  } else if ((l = listener_lookup(p)) != NULL) {
    listener_packet(l, p, &opts, flow_group);
  } else {
    ret = -1;
      ...
      (TCPH_FLAGS(&p->tcp) & TCP_FIN) == TCP_FIN)
  {
    ...
    * why necessary*/
    send_control(c, TCP_ACK, 1, 0, 0);
      ...
      (TCPH_FLAGS(&p->tcp) & TCP_RST) == TCP_RST)
  {
    tcp_close(c);
  } else {
    fprintf(stderr, "%s: unexpected connection state %u flags %hx\n", __func__, c->status, TCPH_FLAGS(&p->tcp));
  }
}
    ...
    const struct tcp_opts *opts)
{
  ...
  if ((TCPH_FLAGS(&p->tcp) & (TCP_SYN | TCP_ACK)) != (TCP_SYN | TCP_ACK)) {
    fprintf(stderr, "%s: unexpected flags %x\n",
        __func__, TCPH_FLAGS(&p->tcp));
    return -1;
  }
  if (opts->ts == NULL) {
    fprintf(stderr, "%s: no timestamp option received\n", __func__);
    return -1;
  }
  ...
  if (nicif_connection_add(c->db_id, c->remote_mac, c->local_ip, c->local_port,
        ...
        c->flow_group, &c->flow_id)
      != 0)
  {
    fprintf(stderr, "%s: nicif_connection_add failed\n", __func__);
    return -1;
  }
...
static inline struct connection *conn_alloc(void)
{
  ...
  conn->rx_buf = (uint8_t *) flextoe_dma_mem + off_rx;
  conn->rx_len = config.tcp_rxbuf_len;
  conn->tx_buf = (uint8_t *) flextoe_dma_mem + off_tx;
...
static void conn_close_timeout(struct connection *c)
{
  ...
  /* free connection id */
  nicif_connection_free(c->flow_id, c->flow_group);
    ...
    uint16_t mss_opt)
{
  uint32_t new_tail;
  struct pkt_tcp *p;
  struct tcp_noop_opt *opt_noop1;
  struct tcp_noop_opt *opt_noop2;
  struct tcp_mss_opt *opt_mss;
  struct tcp_timestamp_opt *opt_ts;
  uint8_t optlen;
  uint16_t len, off_ts, off_mss, off_noop1, off_noop2;

  /* calculate header length depending on options */
  optlen = 0;
  off_noop1 = optlen;
  optlen += sizeof(*opt_noop1);
  off_noop2 = optlen;
  optlen += sizeof(*opt_noop2);
  off_ts = optlen;
  ...
  opt_noop1 = (struct tcp_noop_opt *) ((uint8_t *) (p + 1) + off_noop1);
  opt_noop1->kind = TCP_OPT_NO_OP;
  opt_noop2 = (struct tcp_noop_opt *) ((uint8_t *) (p + 1) + off_noop2);
  opt_noop2->kind = TCP_OPT_NO_OP;
  ...
  /* send packet */
  nicif_tx_send(new_tail, !ts_opt, sizeof(struct pkt_tcp) + off_ts + offsetof(struct tcp_timestamp_opt, ts_val));
  return 0;
}
  ...
  while (off < opts_len) {
    ...
    if (opt_kind == TCP_OPT_END_OF_OPTIONS) {
      ...
      if (opt_kind == TCP_OPT_MSS) {
        ...
        fprintf(stderr, "parse_options: invalid option (kind %u "
            "len %u)\n", opt_kind, opt_len);
        return -1;
      }
    }
