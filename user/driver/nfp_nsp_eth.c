...
static const struct {
	...
	{ RATE_40G, SPEED_40000, },
	{ RATE_50G, SPEED_50000, },
	{ RATE_100G, SPEED_100000, },
};
 ...
 * @tx_enable: Desired TX state
 * @rx_enable: Desired RX state
 *
 ...
 */
int
nfp_eth_set_mod_enable(struct nfp_cpp *cpp, unsigned int idx,
					int enable, int tx_enable, int rx_enable)
{
	union eth_table_entry *entries;
	struct nfp_nsp *nsp;
	uint64_t reg;
	...
	if (tx_enable != (int)FIELD_GET(NSP_ETH_CTRL_TX_ENABLED, reg)) {
		reg = rte_le_to_cpu_64(entries[idx].control);
		reg &= ~NSP_ETH_CTRL_TX_ENABLED;
		reg |= FIELD_PREP(NSP_ETH_CTRL_TX_ENABLED, tx_enable);
		entries[idx].control = rte_cpu_to_le_64(reg);

		nfp_nsp_config_set_modified(nsp, 1);
	}

	if (rx_enable != (int)FIELD_GET(NSP_ETH_CTRL_RX_ENABLED, reg)) {
		reg = rte_le_to_cpu_64(entries[idx].control);
		reg &= ~NSP_ETH_CTRL_RX_ENABLED;
		reg |= FIELD_PREP(NSP_ETH_CTRL_RX_ENABLED, rx_enable);
		entries[idx].control = rte_cpu_to_le_64(reg);
