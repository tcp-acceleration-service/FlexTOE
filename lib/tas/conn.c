...
int flextcp_listen_open(struct flextcp_context *ctx,
    ...
    uint32_t flags)
{
  uint32_t pos = ctx->spin_head;
  struct sp_appout *spin = ctx->spin_base;
  uint32_t f = 0;
  ...
  if ((flags & FLEXTCP_LISTEN_REUSEPORT) == FLEXTCP_LISTEN_REUSEPORT) {
    f |= SP_APPOUT_LISTEN_REUSEPORT;
  }

  spin += pos;

  if (spin->type != SP_APPOUT_INVALID) {
  ...
  spin->data.listen_open.opaque = OPAQUE(lst);
  spin->data.listen_open.local_port = port;
  spin->data.listen_open.backlog = backlog;
  spin->data.listen_open.flags = f;
  MEM_BARRIER();
  spin->type = SP_APPOUT_LISTEN_OPEN;
  flextcp_sp_kick();
    ...
    struct flextcp_listener *lst, struct flextcp_connection *conn)
{
  ...
  spin->data.accept_conn.listen_opaque = OPAQUE(lst);
  spin->data.accept_conn.conn_opaque = OPAQUE(conn);
  spin->data.accept_conn.local_port = lst->local_port;
  MEM_BARRIER();
  spin->type = SP_APPOUT_ACCEPT_CONN;
  flextcp_sp_kick();
...
{
  uint32_t pos = ctx->spin_head, f = 0;
  struct sp_appout *spin = ctx->spin_base;
  ...
  spin->data.conn_open.opaque = OPAQUE(conn);
  spin->data.conn_open.remote_ip = dst_ip;
  spin->data.conn_open.remote_port = dst_port;
  spin->data.conn_open.flags = f;
  MEM_BARRIER();
  spin->type = SP_APPOUT_CONN_OPEN;
  flextcp_sp_kick();
...
#if 0
  if (reset)
    f |= SP_APPOUT_CLOSE_RESET;
#endif

  conn->status = CONN_CLOSE_REQUESTED;

  spin->data.conn_close.opaque = (uintptr_t) conn;
  spin->data.conn_close.remote_ip = conn->remote_ip;
  spin->data.conn_close.remote_port = conn->remote_port;
  spin->data.conn_close.local_ip = conn->local_ip;
  spin->data.conn_close.local_port = conn->local_port;
  spin->data.conn_close.flags = f;
  MEM_BARRIER();
  spin->type = SP_APPOUT_CONN_CLOSE;
  flextcp_sp_kick();
        ...
        struct flextcp_connection *conn)
{
  ...
  spin->data.conn_move.local_ip = conn->local_ip;
  spin->data.conn_move.remote_ip = conn->remote_ip;
  spin->data.conn_move.local_port = conn->local_port;
  spin->data.conn_move.remote_port = conn->remote_port;
  spin->data.conn_move.db_id = ctx->db_id;
  spin->data.conn_move.opaque = OPAQUE(conn);
  MEM_BARRIER();
  spin->type = SP_APPOUT_CONN_MOVE;
  flextcp_sp_kick();
