...
#define DMA_RX_INFLIGHT_PER_ME  (4 * 8)
/* Reserve PCIe Resources */
PCIE_DMA_ALLOC(rx, me, 0, topci_lo, DMA_RX_INFLIGHT_PER_ME);
          ...
          SIGNAL* sig)
{
  ...
  if ((fwd->flags & WORK_RESULT_TX) != 0) {
    ...
    pkt_nbi_drop_seq(fwd->isl + 32, fwd->pnum, &msi,
...
#define FETCH(X)    __mem_workq_add_thread(rx_dma_rnum, rx_dma_raddr_hi, &rx_cmd[X], sizeof(struct dma_pkt_cmd_t), sizeof(struct dma_pkt_cmd_t), sig_done, &rx_sig ## X)
#define BACKUP(X)   backup_fwd_info(&rx_cmd[X], &rx_backup[X])
#define ISSUE(X)    issue_pkt_dma(DMA_RX_PCIe_QUEUE, &rx_cmd[X], &dma_cmd[X], dma_cmd_word1, dma_cmd_word3, &dma_sig ## X)
#define FORWARD(X)  issue_fwd(&rx_backup[X], &dma_cmd[X], &dma_sig ## X)

#define WAIT(X, Y)  __wait_for_any(&rx_sig ## X, &dma_sig ## Y)
#define WAIT_CMD(X) __wait_for_any(&rx_sig ## X)
...
{
  /* Transfer registers */
  __xread struct dma_pkt_cmd_t rx_cmd[4];
  __xwrite struct nfp_pcie_dma_cmd dma_cmd[4];
  SIGNAL rx_sig0, rx_sig1, rx_sig2, rx_sig3;
  SIGNAL dma_sig0, dma_sig1, dma_sig2, dma_sig3;

  /* Backup commands to issue forward */
  __gpr struct fwd_info_t rx_backup[4];
...
int main()
{
  ...
  if (ctx() == 0) {
    dma_cfg_setup(DMA_RX_CFG_IDX);
    dma_cmd_word3 = DMA_CMD_WORD3(RX);

    rx_dma_rnum = MEM_RING_GET_NUM(dma_rx_ring);
    rx_dma_raddr_hi = MEM_RING_GET_MEMADDR(dma_rx_ring);

    msi = pkt_gen_msi_info(PKT_NBI_OFFSET + MAC_PREPEND_BYTES - 4);
